...
from api.threading_api import audio_translate, text_translate, waiting_times, stop_thread, sse_audio_translate
...
async def change_transcription_model(model_name: str = Form(...)):  
    ...
    if not hasattr(model.models_path, model_name):  
        ...
        return BaseResponse(status="FAILED", message=f" | Model '{model_name}' not found. | ", data=None)  
    ...
    if message is None:  
        return BaseResponse(status="OK", message=f" | Model {model_name} has been loaded successfully. | ", data=None)  
    else:  
        return BaseResponse(status="FAILED", message=f" | Model {model_name} loading failed: {message} | ", data=None)  
...
async def translate(
    ...
    try:  
        ...
        if not result_queue.empty():  
            result, rtf, transcription_time, translate_time, translate_method = result_queue.get()  
            ...
            logger.info(f" | device_id: {response_data.device_id} | audio_uid: {response_data.audio_uid} | original language: {o_lang} | translate_method: {translate_method} |")  
            logger.info(f" | ZH: {response_data.text.get('zh', '')} | ")  
            logger.info(f" | EN: {response_data.text.get('en', '')} | ")  
            logger.info(f" | DE: {response_data.text.get('de', '')} | ")  
            logger.info(f" | RTF: {rtf} | total time: {transcription_time + translate_time:.2f} seconds. | transcribe {transcription_time:.2f} seconds. | translate {translate_time:.2f} seconds. |")  
            state = "OK"  
        else:  
            logger.info(f" | Inference has exceeded the upper limit time and has been stopped |")  
            state = "FAILED"  
  
        return BaseResponse(status=state, message=f" | ZH: {response_data.text.get('zh', '')} | EN: {response_data.text.get('en', '')} | DE: {response_data.text.get('de', '')} | ", data=response_data)  
...
async def text_translate(  
    ...
    try:  
        ...
        # Create timing thread and inference thread  
        time_thread = threading.Thread(target=waiting_times, args=(stop_event, model, WAITING_TIME))  
        inference_thread = threading.Thread(target=text_translate, args=(model, text, result_queue, language, stop_event))  
        ...
        if not result_queue.empty():  
            ...
            logger.debug(response_data.model_dump_json())  
            logger.info(f" | device_id: {response_data.device_id} | audio_uid: {response_data.audio_uid} | original language: {language} | translate_method: {translate_method} |")  
            logger.info(f" | ZH: {response_data.text.get('zh', '')} | ")  
            logger.info(f" | EN: {response_data.text.get('en', '')} | ")  
            logger.info(f" | DE: {response_data.text.get('de', '')} | ")  
            ...
            while not sse_stop_event.is_set():  
                if waiting_list and not model.processing:  
                    ...
                    try:  
                        ...
                        # Create timing thread and inference thread  
                        time_thread = threading.Thread(target=waiting_times, args=(stop_event, model, WAITING_TIME))  
                        inference_thread = threading.Thread(target=sse_audio_translate, args=(model, audio_buffer, o_lang, stop_event))  
                        ...
                        while not model.result_queue.empty():
                            result, rtf, transcription_time, translate_time, translate_method = model.result_queue.get()  
                            response_data.text = result  
                            response_data.transcribe_time = transcription_time  
                            response_data.translate_time = translate_time  
                            ...
                            logger.info(f" | EN: {response_data.text.get('en', '')} | ")  
                            logger.info(f" | DE: {response_data.text.get('de', '')} | ")  
                            logger.info(f" | RTF: {rtf} | transcribe has been completed in {transcription_time:.2f} seconds. | translate has been completed in {translate_time:.2f} seconds. |")  
                            
                            base_response = BaseResponse(  
                                status="OK",  
                                message=f" | ZH: {response_data.text.get('zh', '')} | EN: {response_data.text.get('en', '')} | DE: {response_data.text.get('de', '')} | ",  
                                data=response_data  
                            )  
